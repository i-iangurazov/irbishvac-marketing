...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("install-evening-report-once", help="Build and optionally send today's install collections and tomorrow's schedule")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "install-evening-report-once":
        app.initialize_storage()
        summary = app.run_install_evening_report_once()
        print("\n".join(summary.to_lines()))
        return 1 if summary.status in {"config_error", "api_error", "slack_error"} else 0
