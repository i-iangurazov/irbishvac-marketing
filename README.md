...
Use `test-email` after configuring SMTP. By default it sends to `TIM_EMAIL` and `VADIM_EMAIL`; pass `--to you@example.com` to send only to a specific address.
...
Those commands may post to Slack and, for Friday roundup, attempt email. `test-email` only sends SMTP email and does not read Notion or post to Slack.
