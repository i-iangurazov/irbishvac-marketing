...
| R6 | Comfort Advisor set | Yes | Sold-by/Comfort Advisor | Project custom field `Sold by`; jobs also expose `soldById` | Ready | Low if custom field is consistently used | Implement in v1 |
| R7 | Permit number present | Yes | Permit value | Project custom field `Permit` | Ready for field presence | Medium until value semantics are checked | Implement in v1 dry-run first |
