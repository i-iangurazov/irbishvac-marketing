...
- `plumbing_options_fewer_than_three`: closed Plumbing Service diagnostic jobs must show at least three estimate/option records only after the business confirms this expectation for positive-invoice diagnostic work. The rule excludes `57804592 / Water Heater Maintenance` pending business confirmation and returns `not_applicable` for structured zero-dollar/no-charge jobs where invoice total and balance are both `0`. If billing context is unavailable or does not show a positive charge, the rule returns `insufficient_data` rather than failing.
