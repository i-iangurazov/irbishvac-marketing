...
The v1 implementation uses project type first. If data is missing or uncertain, rules return `skip` rather than failing.
...
| R3 | PM assigned | Fails when no PM is assigned after `PM_AUDIT_PM_ASSIGNMENT_GRACE_HOURS`. |
| R6 | Comfort Advisor / Sold By set | Fails when `Sold by` custom field is available and empty. |
| R7 | Permit field present | Fails when `Permit` custom field is available and empty. |
| R11 | Tasks applied / task count present | Fails when no project tasks exist after `PM_AUDIT_TASK_TEMPLATE_GRACE_HOURS`. |
| R13 | Every task has an assignee | Fails when any project task has no assignee. |
| R15 | No stale open tasks | Fails when an open task is more than `PM_AUDIT_TASK_OVERDUE_DAYS` past due. |
